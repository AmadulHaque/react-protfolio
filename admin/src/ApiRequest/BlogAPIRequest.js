...
import {SetblogList, SetblogTotal,blogDetailsValue} from "../redux/state-slice/blog-slice";
...
export async function ListRequest(pageNo, perPageKey, searchKeyword) {
    try {
        ...
        let URL = BaseURl+"/BlogList/"+pageNo+"/"+perPageKey+"/"+searchKeyword;
        const result = await axios.get(URL,AxiosHeader)
        store.dispatch(HideLoader())
        if (result.status === 200 && result.data['status'] === "success") {
            if (result.data['data'][0]['Rows'].length > 0) {
                store.dispatch(SetblogList(result.data['data'][0]['Rows']))
                store.dispatch(SetblogTotal(result.data['data'][0]['Total'][0]['count']))
            } else {
                store.dispatch(SetblogList([]))
                store.dispatch(SetblogTotal(0))
...
export async function DetailsGet(ObjectID) {
    try {
        store.dispatch(ShowLoader())
        let URL = BaseURl+"/BlogDetailsByID/"+ObjectID;
        const result = await axios.get(URL,AxiosHeader)
        store.dispatch(HideLoader())
        if (result.status === 200 && result.data['status'] === "success") {
            let data =result.data['data'][0];
            store.dispatch(blogDetailsValue(data));
            return  true;
        } else {
            debugger;
            ErrorToast("Request Fail ! Try Again")
